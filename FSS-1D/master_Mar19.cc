...
//	int currentLoads[noSlaves];	//loads of slaves
	...
	float newChunk;
		...
		if((remainingChunks >= tempChunk) && (tempChunk > threshold))
		{
			for(j = 1; j <= noSlaves; j++)
			{
			    ...
			    thisStageChunkSize = pow(0.5,(i+1))*((DOTSX*availablePowers[j])/totalPower);
				cout <<"power(0.5,"<<i<<"+1):"<<pow(0.5,(i+1))<<"  Chunk:"<<thisStageChunkSize<< endl;

			//	cout <<"Chunk:"<<thisStageChunkSize << endl;
				/* Ceiling function */
			        if (thisStageChunkSize-(int)thisStageChunkSize>0)
		...
		{
			if(remainingChunks <= (totalPower*threshold))
			{
				for(j = 1; j <= noSlaves; j++)
				{
