...
//	int currentLoads[noSlaves];	//loads of slaves
	...
	float newChunk;
		...
		int remainder = remainingChunks;
		//thisStageChunkSize = (float)(remainingChunks) / (float)(2*totalPower);

		thisStageChunkSize = pow(0.5,(i+1))*(DOTSX/totalPower);
                ...
                else
		{
		     ...
		     cout <<"ChunkequaDivide:"<<currentChunk<<endl;
			...
			int stage = i;
				...
				stage++;
               			for(j=1;j<=noSlaves;j++)
				{
                       			roundsTable[stage*noSlaves + j] = (long)remainder;
                       			remainder = 0;
                                        remainingChunks = 0;
