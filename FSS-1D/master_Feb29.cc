...
//	int currentLoads[noSlaves];	//loads of slaves
	...
	roundsTable = new long[MAXROUNDS*noSlaves + 1];
		...
		if(remainingChunks < 2*totalPower*threshold)
                {
                      for(j = 1; j <= noSlaves; j++)
                      {
                             thisStageChunkSize = (float)(remainingChunks*availablePowers[j]) / (float)(totalPower);	
			/* If the total chunk assigning is less than or equal tothe remaining chunks */
		             if((chunkAssignedInThisStage + (int)thisStageChunkSize) <= remainingChunks)

                             {
                                /* Ceiling function */
                                if (thisStageChunkSize-(int)thisStageChunkSize>0)
                                        currentChunk = (int)thisStageChunkSize + 1;
                                else
                             ...
                             {
                                currentChunk = remainingChunks - chunkAssignedInThisStage;
                             }
                ...
                else
                {
                        for(j = 1; j <= noSlaves; j++)
                        {
                                thisStageChunkSize = (float)(remainingChunks*availablePowers[j]) / (float)(2*totalPower);
				/* ceiling function */
                                if (thisStageChunkSize-(int)thisStageChunkSize>0)
                                        currentChunk = (int)thisStageChunkSize + 1;
                                else
                                ...
                                roundsTable[i*noSlaves + j] = (long)currentChunk;
                                //cout << "roundsTable["<<(i*noSlaves+j) << "]: "<< roundsTable[i*noSlaves+j]<< endl;

                                chunkAssignedInThisStage += roundsTable[i*noSlaves + j];
                        }
                remainingChunks -= chunkAssignedInThisStage;

                //cout <<"chunkAssignedInThisStage: "<<chunkAssignedInThisStage<<", remaining Chunks: "<<remainingChunks<< endl;
