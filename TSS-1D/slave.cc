...
Slave::Slave(int myId)
{
	...
	float *putData;
  	...
  	int dimParameter = maxPower*(DOTSX*DOTSY)/(2*totPower);

//	putData = (float*) calloc (DOTSX*DOTSY+100, sizeof(float));//This is the old one

//	putData = (float*) calloc (dimParameter+10, sizeof(float));

	// It is possible to use both calloc or new	
...
*/
			...
			// if we use 3 after putData inside the MPI_Send: it is without returning results	
			//if we use counter after putData inside the MPI_Send: it is returning results     
			MPI_Send(putData, counter, MPI_FLOAT, 0, 3, MPI_COMM_WORLD);
