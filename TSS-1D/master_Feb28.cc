...
void Master::workCalculate(int slaveId)
{
	...
	if(FChunk){
		ngoal=(int) ((firstChunk*availablePowers[slaveId])-(availablePowers[slaveId]*delta*((availablePowers[slaveId]-1)/2)) );
		currentChunk=ngoal;
	}
	else{
...
//	int currentLoads[noSlaves];	//loads of slaves
	...
	cout << "Number of chunnks: "<< noChunks << endl;
