...
void Master::workCalculate(int slaveId)
{
	...
	startCol = assignedColumns;
       ...
       // Explanation for these formula in given in the file "explanation.txt" 
			
	if(FChunk){
		currentChunk=(int) ((firstChunk*availablePowers[slaveId])-(availablePowers[slaveId]*delta*((availablePowers[slaveId]-1)/2)) );
		//currentChunk=ngoal;
	}
	else{
		currentChunk=(int)(-(availablePowers[slaveId]*delta*((availablePowers[slaveId]+prevAPower)/2))+
			((availablePowers[slaveId]/prevAPower)*currentChunk));	
		//ngoal=currentChunk;	
	...
	// Threshold is the last chunk size we allow
        if((unassignedColumns >= currentChunk) && (currentChunk > threshold))
        {
                ngoal = currentChunk;
        }
        else // if (currentChunk <= threshold)
        {
		if(unassignedColumns >= (powerMax*threshold))
		{
			ngoal = (int)(threshold * availablePowers[slaveId]);
			chunkCount++;
		}
		else
		{
			if((threshold <= unassignedColumns)&&(unassignedColumns < (powerMax * threshold)))
			{
				if(availablePowers[slaveId] == powerMax)
					ngoal = unassignedColumns;// assign the remaining chunks
				else
				{// assign the threshold as the current chunk
					ngoal = threshold;
					chunkCount++;
				}
			}	
			else // assigne the remaining chunks
...
//	int currentLoads[noSlaves];	//loads of slaves
	...
	cout << "Number of chunks: "<< noChunks << endl;
	...
	cout << "The number of scheduling steps: "<<scount << endl;	
	cout << "The number of same chunk scheduling steps: "<<chunkCount<< endl;

	outFile << "The number of scheduling steps: "<< scount << endl;
	outFile << "The number of same chunk scheduling steps: "<<chunkCount << endl;
