		 ...
		 if(availablePowers[slaveId] >= 1){
			...
			for(int i=1; i<=availablePowers[slaveId]; i++) 
			{
			  //  temp = (float)(Remainder) / (float)(totalPower);
			   ...
			   temp = pow((1-(1/totalPower)),j-1)*(DOTSX/totalPower);
			    cout <<"slave "<<slaveId<<"Actual chunk:" << temp<<endl;
			    ...
			    Remainder = Remainder - tempChunk;// Remaining chunks
			    totalChunks += tempChunk; // total chunk assigned for this slave
			}
			tempChunk = totalChunks;
		    }
		    else // if gss
	            {
			 ...
			 cout << "slave "<<slaveId<<" chunk:"<< temp<< endl;
			...
			if((unassignedColumns >= tempChunk) && (tempChunk > threshold))
			{
				chunk.noOfColumns = tempChunk;
			}
			else // if(tempChunk <= threshold)
			{
				if(unassignedColumns >= (powerMax * threshold))
				{
					chunk.noOfColumns = (int) (threshold * availablePowers[slaveId]);
					chunkCount ++;
                    		}
                    		else 
                    		{
					if((threshold <= unassignedColumns)&&(unassignedColumns < (powerMax * threshold)))
					{
						if(availablePowers[slaveId] == powerMax){
							chunk.noOfColumns = unassignedColumns; //assign the remaining chunks
						}
						else
						{
						     chunk.noOfColumns = threshold;
						     chunkCount++;
						}
					}
					else{ // assign the remaining chunks
