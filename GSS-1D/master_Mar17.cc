...
//	int currentLoads[noSlaves];	//loads of slaves
	...
	while (1) 
	{
		...
		scomm[slaveId] = MPI_Wtime();
	    	...
	    	{
		    int chunkForThisRound = 0;		
		    int totalSlaves = noSlaves; // noSlaves is from file input.h
		    int Remainder = unassignedColumns;
		 if(availablePowers[slaveId] > 0.2){
			...
			while( totalSlaves != 0)
			{
			    ...
			    Remainder = Remainder - tempChunk;
			    chunkForThisRound = chunkForThisRound + tempChunk;
			    totalSlaves--;  
			}
			tempChunk = chunkForThisRound;
		    }
		    else
	            {
