...
//	int currentLoads[noSlaves];	//loads of slaves
	...
	getData = new float[size+100];

        int i = 1; //number of steps
	...
	while (1) 
	{
		...
		scomm[slaveId] = MPI_Wtime();
		 ...
		 if(availablePowers[slaveId] >= 1){
		    ...
		    {
			for(int j=1; j<=availablePowers[slaveId]; j++) 
			{
			    temp = pow(((totalPower-1)/totalPower),((i-1)+(j-1)))*(DOTSX/totalPower);

			    //cout <<"slave "<<slaveId<<"power:"<<availablePowers[slaveId]<<" Chunk:"<<temp<<endl; 
			    //ceiling function
			    if(temp-(int)temp > 0)
			...
			chunk.noOfColumns = totalChunks;
	            ...
	            {
			 temp = pow((1-(1/totalPower)),i-1)*(DOTSX/totalPower);
